...
public class Model_Employee
{
	...
	public Model_Employee save()
	{
		try
		{
			...
			map.put("Salary" , this.get("Salary"));
			map.put("BirthDate" , this.get("BirthDate"));
			map.put("Address" , this.get("Address"));
			map.put("Gender" , this.get("Gender"));
			
			String id = this.get("employeeId");
			if(id.equals("-1"))
			{
				this.getAdapter().insert("Employee" , map);
			}
			else
			{
				this.getAdapter().update("Employee" , "employeeId" , id , map);
			}
		}
		catch(Exception e)
		{
