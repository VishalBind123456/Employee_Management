...
<script>
	jQuery("#add").on({
		click : function(){
			...
			url = "http://localhost:8080/Employee_Management/controller_employee?action=upsert&employeeId=-1";
	...
	jQuery(".edit").on({
		click : function(){
			var empId = jQuery(this).val();
			url = "http://localhost:8080/Employee_Management/controller_employee?employeeId="+empId+"&action=upsert";
			window.location.href = url;
		}
	});
	jQuery(".delete").on({
		click : function(){
			...
			if(resp == true)
			{
				url = "http://localhost:8080/Employee_Management/controller_employee?employeeId="+empId+"&action=delete";
				window.location.href = url;				
			}
